...
def generar_cronograma_sennova(
    ...
    try:
        ...
        # Solo admin o owner (aprendices no pueden generar cronogramas)
        if current_user.rol == "aprendiz":
            raise HTTPException(status_code=403, detail="Los aprendices no tienen permiso para generar cronogramas")
...
def generar_detalle_presupuesto(
    ...
    try:
        ...
        # Calcular nivel de ejecución real basado en entregables aprobados
        ...
        nivel_ejecucion = round((aprobados / total_e * 100), 1) if total_e > 0 else 0.0
        
        # La eficiencia operativa es una métrica de gestión, la basaremos en el cumplimiento de fechas
        vencidos_o_atrasados = 0
        hoy = date.today()
        for e in entregables:
            if e.estado != 'aprobado' and e.fecha_entrega < hoy:
                vencidos_o_atrasados += 1
        
        eficiencia = round(100 - (vencidos_o_atrasados / total_e * 100), 1) if total_e > 0 else 100.0
