...
def test_sennova_automation_flow():
    ...
    try:
        ...
        if response.status_code != 200:
            pytest.skip(f"Omitiendo test de SENNOVA: El backend en {BASE_URL} no respondió 200 en login ({response.status_code})")
            return
    except requests.exceptions.RequestException as e:
        pytest.skip(f"Omitiendo test de SENNOVA: El backend no está activo o cerró conexión en {BASE_URL} ({e})")
        return
    ...
    try:
        ...
        pytest.skip(f"Omitiendo resto del test de integración externa: Conexión interrumpida con el backend ({e})")
